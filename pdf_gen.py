...
def generate_calmera_pdf(data):
    ...
    overall_score_val = data.get('overall_score', 0)
    grade = data['grade']
    ...
    water_score = data.get('water_score_percent', 0)
    sanitation_score = data.get('sanitation_score_percent', 0)
    ...
    pdf.body_text(f"Water Readiness: {data.get('water_score_percent', 0):.0f}% — {'Fully covered' if data.get('water_score_percent', 0) >= 80 else 'Partial coverage'}")
    pdf.body_text(f"Power Backup: {data.get('power_score_percent', 0):.0f}% — {'Full backup' if data.get('power_score_percent', 0) >= 80 else 'Partial coverage'}")
    pdf.body_text(f"Sanitation Plan: {data.get('sanitation_score_percent', 0):.0f}% — {'System in place' if data.get('sanitation_score_percent', 0) >= 80 else 'Missing'}")
    pdf.body_text(f"Communications: {data.get('comms_score_percent', 0):.0f}% — {'Complete' if data.get('comms_score_percent', 0) >= 80 else 'Gaps to address'}")
    pdf.ln(4)
